...
const updatePage = async () => {
  ...
  const sugarArray = fruitsArray.filter(fruit => fruit.nutritions.sugar > 15).map(fruit => fruit.name);
  console.log(sugarArray)

  // TODO: Create a new HTML element to display your data
  const fruitElement = document.createElement("fruit-data");
  fruitElement.innerHTML = sugarArray.join([separator = ', ']);


  // TODO: Append your new element to the page
  const galleryElement = document.getElementById("cs1300-gallery");
  galleryElement.append(fruitElement);
